...
// Get current role (fallback to user_type if role column doesn't exist)
$current_role = $has_role_column && isset($user['role']) ? $user['role'] : $user['user_type'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        // IMPORTANT: Keep user_type as 'admin' so user stays visible in user management
        $user_type = 'admin';
        ...
        if (!empty($password)) {
            ...
            if ($has_role_column) {
                $stmt = $conn->prepare("UPDATE users SET full_name = ?, email = ?, password = ?, user_type = ?, role = ?, status = ? WHERE id = ?");
                $stmt->bind_param("ssssssi", $full_name, $email, $hashed_password, $user_type, $role, $status, $user_id);
            } else {
                $stmt = $conn->prepare("UPDATE users SET full_name = ?, email = ?, password = ?, user_type = ?, status = ? WHERE id = ?");
                $stmt->bind_param("sssssi", $full_name, $email, $hashed_password, $user_type, $status, $user_id);
            }
        } else {
            // Update without password
            if ($has_role_column) {
                $stmt = $conn->prepare("UPDATE users SET full_name = ?, email = ?, user_type = ?, role = ?, status = ? WHERE id = ?");
                $stmt->bind_param("sssssi", $full_name, $email, $user_type, $role, $status, $user_id);
            } else {
                $stmt = $conn->prepare("UPDATE users SET full_name = ?, email = ?, user_type = ?, status = ? WHERE id = ?");
                $stmt->bind_param("ssssi", $full_name, $email, $user_type, $status, $user_id);
            }
        }
        
        if ($stmt->execute()) {
            // Log activity
            log_activity($_SESSION['user_id'], 'user_updated', "Updated user: $full_name (ID: $user_id, Role: $role)");
        ...
        <div class="user-header">
            ...
            <div class="user-meta">
                ...
                <div class="user-badges">
                    <span class="role-badge role-<?php echo $current_role; ?>">
            ...
            <div class="form-grid">
                <div class="form-section">
                    ...
                    <div class="info-box">
                        ...
                        <strong>Username:</strong> <?php echo htmlspecialchars($user['username']); ?> (cannot be changed)
                    </div>
                </div>
                ...
                <div class="form-section">
                    ...
                    <div class="form-group">
                        ...
                        <select id="role" name="role" required 
                            ...
                            <option value="admin" <?php echo ($current_role === 'admin') ? 'selected' : ''; ?>>
                                Administrator
                            </option>
                            <option value="staff" <?php echo ($current_role === 'staff') ? 'selected' : ''; ?>>
                                Staff
                            </option>
                        </select>
